# Power-Bi-Practice-Dashboards

A collection of Power BI practice dashboards showcasing data cleaning, visualization, KPI design, and business insights using real-world datasets.

---

## Dashboards

### 1. Amazon India Sales Dashboard
![Amazon Sales Dashboard](https://raw.githubusercontent.com/Maanu-Hue/Power-Bi-Practice-Dashboards/main/Screenshot%202026-06-06%20222736.png)
- **Files:** `AMAZON SALES ANALYTICS DASHBOARD.pbix`, `Amazonsales.xlsx`
- **Tools:** Power BI, Excel
- Explores sales performance, product categories, and revenue trends across Amazon India data.

---

### 2. IBM HR Attrition Dashboard
![IBM HR Dashboard](https://raw.githubusercontent.com/Maanu-Hue/Power-Bi-Practice-Dashboards/main/Screenshot%202026-06-06%20222754.png)
- **Files:** `IBM HR Analysis.pbix`, `IBM HR Employee.xlsx`
- **Tools:** Power BI, Excel
- Analyzes employee attrition patterns using the IBM HR dataset. Covers department-wise attrition, age groups, job satisfaction, and KPIs.

---

### 3. Sales Data Dashboard
![Sales Dashboard](https://raw.githubusercontent.com/Maanu-Hue/Power-Bi-Practice-Dashboards/main/Screenshot%202026-06-06%20223051.png)
- **Files:** `Sales_Data_Dashboard.pbix`, `Sales_Analysis.xlsx`
- **Tools:** Power BI, Excel
- Visualizes sales trends, regional performance, and revenue metrics from a retail sales dataset.

---

### 4. Hotel Booking Dashboard
![Hotel Booking Dashboard](https://raw.githubusercontent.com/Maanu-Hue/Power-Bi-Practice-Dashboards/main/Screenshot%202026-06-08%20223015.png)
- **File:** `hotel_booking.pbix`
- **Tools:** Power BI
- Built on a 119K-record hotel booking dataset. Covers booking trends, cancellation rates, lead time analysis, and customer segmentation.

---

### 5. Flights Airport Dashboard
![Flights Airport Dashboard](https://raw.githubusercontent.com/Maanu-Hue/Power-Bi-Practice-Dashboards/main/Screenshot%202026-06-22%20235024.png)
- **File:** `flightsairport.pbix`
- **Tools:** Power BI
- Analyzes flight and airport data including route performance, delay patterns, and airport-level insights.

---

## Skills Demonstrated
- Data cleaning and transformation using Power Query
- DAX measures and calculated columns
- KPI cards, slicers, and interactive filter panes
- Multi-page report design with navigation buttons
- Dashboard storytelling for business use cases

---

## Tools Used
- Microsoft Power BI Desktop
- Microsoft Excel

---

## Connect With Me
- 💼 [LinkedIn](https://www.linkedin.com/in/manasadataanalyst)
- 🐙 [GitHub](https://github.com/Maanu-Hue)

---

*Built as part of my data analytics learning journey. Each dashboard reflects hands-on practice with real or realistic datasets.*
